...
export default function SupportModule({ onBack }: Props) {
  ...
  const sendComment = () => {
    ...
    const comment = { author: "Вы", text: commentText + (attachedFile ? ` [📎 ${attachedFile}]` : ""), time: "только что" };
    setTickets(prev => prev.map(t => t.id !== selectedId ? t : { ...t, comments: [...t.comments, comment] }));
    setCommentText("");
  ...
  const createTicket = () => {
    ...
    setAttachedFile(null);
    setView("list");
    showToast("✅ Обращение отправлено! Мы ответим в течение 4 часов.");
  };
        ...
        <div className="max-w-2xl mx-auto px-4 pt-4 pb-8 space-y-4">
          ...
          <div className="space-y-3">
            {selected.comments.map((c, i) => (
              <div key={i} className="p-4 rounded-2xl animate-fade-up opacity-0" style={{ animationDelay: `${i * 0.04}s`, animationFillMode: 'forwards', background: c.author === "Вы" ? 'rgba(16,185,129,0.08)' : 'rgba(27,111,255,0.08)', border: `1px solid ${c.author === "Вы" ? 'rgba(16,185,129,0.2)' : 'rgba(27,111,255,0.2)'}` }}>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-semibold text-white/80">{c.author}</span>
                    {c.author === "Вы" && <span className="tag text-xs" style={{ background: 'rgba(16,185,129,0.15)', borderColor: 'rgba(16,185,129,0.3)', color: '#10b981' }}>Вы</span>}
                  </div>
          ...
          {attachedFile && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl" style={{ background: 'rgba(27,111,255,0.1)', border: '1px solid rgba(27,111,255,0.25)' }}>
              ...
              <button onClick={() => setAttachedFile(null)}><Icon name="X" size={14} color="rgba(255,255,255,0.4)" /></button>
            </div>
          )}
          ...
          <div className="flex gap-2">
            ...
            />
            <button onClick={() => setAttachedFile("скриншот.png")} className="p-3 rounded-xl flex-shrink-0 transition-all active:scale-90" style={{ background: attachedFile ? 'rgba(27,111,255,0.3)' : 'rgba(27,111,255,0.15)', border: '1px solid rgba(27,111,255,0.3)' }}>
              <Icon name="Paperclip" size={18} color={attachedFile ? "white" : "rgba(255,255,255,0.5)"} />
            </button>
            <button onClick={sendComment} disabled={!commentText.trim()} className="p-3 rounded-xl flex-shrink-0 transition-all active:scale-90 disabled:opacity-40" style={{ background: 'linear-gradient(135deg, #ec4899, #db2777)' }}>
    ...
    return (
      <div className="min-h-screen relative z-10 animate-fade-in">
        ...
        <div className="max-w-2xl mx-auto px-4 pt-4 pb-8 space-y-4">
          ...
          <button
            onClick={() => setAttachedFile(attachedFile ? null : "скриншот.png")}
            className="w-full flex items-center gap-3 p-4 rounded-xl transition-all"
            style={{ background: attachedFile ? 'rgba(27,111,255,0.12)' : 'rgba(255,255,255,0.04)', border: attachedFile ? '1px solid rgba(27,111,255,0.35)' : '1px dashed rgba(255,255,255,0.15)' }}
          >
            <Icon name="Paperclip" size={18} color={attachedFile ? "#4d8fff" : "rgba(255,255,255,0.4)"} />
            <span className="text-sm" style={{ color: attachedFile ? '#4d8fff' : 'rgba(255,255,255,0.4)' }}>
              {attachedFile ? `✓ ${attachedFile} прикреплён` : "Прикрепить скриншот или файл"}
            </span>
            {attachedFile && <Icon name="X" size={14} color="rgba(255,255,255,0.4)" className="ml-auto" />}
          </button>
          <button onClick={createTicket} className="btn-primary flex items-center justify-center gap-2" disabled={!form.title || !form.desc}>
  ...
  return (
    <div className="min-h-screen relative z-10 animate-fade-in">
      {toast && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 px-4 py-3 rounded-2xl text-sm font-medium text-white animate-fade-up" style={{ background: 'rgba(236,72,153,0.9)', backdropFilter: 'blur(12px)' }}>
          {toast}
        </div>
      )}
      ...
      <div className="max-w-2xl mx-auto px-4 pt-4 pb-8 space-y-4">
        ...
        <div className="space-y-3">
          {tickets.map((t, i) => {
            ...
            return (
              <button key={t.id} onClick={() => { setSelectedId(t.id); setView("ticket"); }} className={`w-full text-left card-module animate-fade-up opacity-0`} style={{ animationDelay: `${0.05 + i * 0.07}s`, animationFillMode: 'forwards' }}>
                <div className="flex items-start justify-between mb-2">
