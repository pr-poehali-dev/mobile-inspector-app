...
export default function ServicesModule({ onBack }: Props) {
  ...
  const executorPrice = Math.round(EXECUTOR_PRICE * (1 - discountPct));
        ...
        <div className="max-w-2xl mx-auto px-4 pt-6 pb-8 space-y-5">
          ...
          <div className="glass rounded-2xl p-4 flex items-center gap-3" style={{ border: refCodeStatus === "valid" ? '1px solid rgba(16,185,129,0.4)' : undefined }}>
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-white">Стоимость доступа</p>
            ...
            <div className="text-right">
              {refCodeStatus === "valid" && <p className="text-xs text-white/30 line-through">{EXECUTOR_PRICE.toLocaleString("ru-RU")} ₽</p>}
              <span className="text-lg font-bold text-green-400">{executorPrice.toLocaleString("ru-RU")} ₽/мес</span>
            </div>
          </div>
          ...
          <div>
            ...
            {refCodeStatus === "valid" && <p className="text-xs text-green-400 mt-1.5 flex items-center gap-1"><Icon name="Check" size={11} color="#10b981" />Код принят — скидка 10% на стоимость</p>}
